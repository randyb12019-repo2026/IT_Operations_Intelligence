...
def predicciones(seccion, predicciones, col_pred):
    ...
    def mostrar_tabla(df, titulo, descripcion=None):
        if len(df) == 0:
    ...
    if len(riesgo) > 0:
        mostrar_tabla(riesgo, "🔴 Tickets en Riesgo",
            f"""
            <div style="font-size:0.85rem; color:#c8d6e5; background:#1a2d44; border-left:3px solid #ff3355; padding:0.75rem 1rem; border-radius:0 4px 4px 0;">
    ...
    mostrar_tabla(predicciones, "📋 Vista General",
        f"""
        <div style="font-size:0.85rem; color:#c8d6e5; background:#1a2d44; border-left:3px solid #2e86c1; padding:0.75rem 1rem; border-radius:0 4px 4px 0;">
